...
class TaskListAdapter : ListAdapter<TaskResponseValue, TaskListAdapter.TaskViewHolder>(DiffCallback()) {
    ...
    inner class TaskViewHolder(
        ...
        fun bind(item: TaskResponseValue, context: Context) {
            ...
            val weekdate = c.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.SHORT, Locale.getDefault())
            ...
            if (item.task_completed == 1L) {
                binding.cvWeekDate.background.setTint(ContextCompat.getColor(context, R.color.ripple_green))
            } else if (item.task_completed == 1L && diferenceDates < 0) {
                binding.cvWeekDate.background.setTint(ContextCompat.getColor(context, R.color.ripple_green))
            } else if (item.task_completed == 0L && diferenceDates < 0) {
                binding.cvWeekDate.background.setTint(ContextCompat.getColor(context, R.color.ripple_red))
            } else {
                binding.cvWeekDate.background.setTint(ContextCompat.getColor(context, R.color.ripple_orange))
            }
        }
